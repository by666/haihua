...
- (id)initWithText:(NSString *)text
{
    if (self = [super init]) {
        ...
        frame.size = CGSizeMake(SCREEN_WIDTH, BTN_HEIGHT);
        self.frame = frame;
...
- (instancetype)initWithTitle:(NSString *)title titleIcon:(UIImage *)icon buttonTitles:(NSArray *)titles
{
    if (self = [super init]) {
        ...
        frame.size = CGSizeMake(SCREEN_WIDTH, 380);
        ...
        if (!IS_NS_STRING_EMPTY(title)) {
            ...
            mTitle = [[IDSExtendedLabel alloc] initWithInsets:UIEdgeInsetsMake(5, 0, 5, 0)];
        ...
        if (nil != titles && [titles count] > 0) {
            ...
            [self showDimMask];
            // title 后面添加分割线
            UIView *endLine = [[UIView alloc] initWithFrame:CGRectMake(0, mStepY, SCREEN_WIDTH, 0.5)];
            ...
            for (int indexCounter = 0; indexCounter < titles.count; indexCounter++) {
                ...
                if (indexCounter <= titles.count - 1) {
                    UIView *btnLine = [[UIView alloc] initWithFrame:CGRectMake(0, mStepY, SCREEN_WIDTH, 0.5)];
...
- (void)showFailure
{
    [self showWithUIColor:WarnColor];
}

- (void)showWithColor:(NSString *)colorHex
{
    mTitle.textColor = [UIColor whiteColor];
    mTitleBgView.backgroundColor = [ColorUtil colorWithHexString:colorHex alpha:1.0f];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    if (self.outerSideCanCancel) {
        __weak IDSAlertSheet *weakSelf = self;
